...
<!-- Latest updates Sep 04, 2023 -->

<!-- Minor edits on the code and its notes for more clarity -->

<!-- Numbering to the sections added to improve the Outline reading -->

<!-- NOTE: Last run on "R version 4.2.1 (2022-06-23 ucrt),
...
df.historical <- data.frame(ID = c('1', '2', '3'),
                      ...
                      End = c('2022_12_31', '2022_12_31', '2022_12_31'))
...
### b. Creating a `.csv` template

Here we export the example tables to "csv" format to use as templates

```{r Export-to-csv-or-Excel}
#1. Create file "Example_input.csv"
write.csv(df.input, 'Example_input.csv', row.names = FALSE, na = '')

#2. Creat file 'Example_input_historical.csv'
write.csv(df.historical, 
          'Example_input_historical.csv', row.names = FALSE, na = '')
...
**Extra variables** <br/> <!-- DS: this would be of interest, i.e., feature engineering, within an agronomic context -->
...
Here we download the daily-weather data from the DAYMET database, and we process it to obtain common variables of agronomic value. <br/>
...
Here we download the daily-weather data from the NASA-POWER database, and we process it to obtain common variables of agronomic value. Finally, we produce a data frame with the requested variables for the specified time period. Check the Tutorial file for the specifics of the formulas. <br/>

Nasapower has recently modified names for many variables. Thus adjustments have been made to this code. We recommend to double check names before processing data <br/>
  ...
  dplyr::mutate(
    Weather = purrr::pmap(list(lat = latitude,
              ...
              function(lat, lon, sta, end) {
                ...
                # Note: Nasapower API used to have PRECTOT; not anymore
    ...
    dplyr::mutate(Weather = Weather %>% 
                    ...
                    # Selecting variables of interest (11):
                    purrr::map(~ dplyr::select(., yday, Year, Month, Day, YYYYMMDD, 
                                               T2M_MIN, T2M_MAX, RH2M, PRECTOTCORR,
                                               SG_DAY_HOURS, ALLSKY_SFC_SW_DWN)) %>% 
                    # Renaming variables (follows same order than the line above):
    ...
    tidyr::unnest(cols = c(Weather)) %>% 
    dplyr::ungroup() %>% 
    # Converting units or creating derived variables:
    dplyr::mutate(Tmean = (Tmax + Tmin)/2, # Mean temp. (degC)
                  ...
                  ws = acos(-tan(lat_rad)*tan(Sd)),
                  Ra = (24*60)/(pi) * Gsc * dr * 
                    (ws*sin(lat_rad)*sin(Sd) + cos(lat_rad)*sin(ws)),
                  ET0_HS = 0.0135 * kRs * (Ra / 2.45) * 
                    (sqrt(Tmax-Tmin)) * (Tmean + 17.8)) %>%
    # remove the unnecessary intermediate variables:
...
weather.chirps <- function(input, dpp = 0){
  ...
  input %>%
    ...
    dplyr::mutate(Weather = Weather %>%
                    ...
                    # NOTE: rename_all() has been superseded by rename_with():
                    purrr::map(~ rename_with(., ~c("DOY", "Year", "Month",
                                                   "Day","Date", "PP"))),
...
years.daymet <- historical.years(hist.data = hist.weather.daymet)
...
# By month...
# Specify hist.data = dataframe containing the historical weather data to summarize
years.months.daymet <- historical.years.months(
  hist.data = hist.weather.daymet)
# historical.summary.daymet.months <-
#   summary.daymet.nasapower(input = hist.weather.daymet,
#                            intervals = years.months.daymet)
...
# Specify hist.data = dataframe containing the historical weather data to summarize
years.nasapower <- historical.years(hist.data = hist.weather.nasapower)
...
# By month
# Specify hist.data = dataframe containing the historical weather data to summarize
years.months.nasapower <- historical.years.months(
  hist.data = hist.weather.nasapower)
# historical.summary.daymet.months <-
#   summary.daymet.nasapower(input = hist.weather.nasapower,
#                            intervals = years.months.nasapower)
...
# Specify hist.data = dataframe containing the historical weather data to summarize
years.chirps <- historical.years(hist.data = hist.weather.chirps)

# input = dataframe containing the historical weather data.
# intervals = type of historical intervals (years, years.months)

# Summarizing historical weather
historical.summary.chirps <- summary.chirps(
  input = hist.weather.chirps, intervals = years.chirps)
...
# By month
# Specify hist.data = dataframe containing the historical weather data to summarize
years.months.chirps <- historical.years.months(hist.data = hist.weather.chirps)

# Summarizing historical weather
historical.summary.chirps.months <- summary.chirps(
  input = hist.weather.chirps, intervals = years.months.chirps)
...
Bootsma, A., S. Gameda, & D.W. McKenney. 2005. Potential impacts of cli- mate change on corn, soybeans and barley yields in Atlantic Canada. *Can J. Soil Sci.* 85:345--357. <https://doi.org/10.4141/S04-025> <br/>
